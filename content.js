...
function removeSubstringFromString(originalString, substringToRemove) {
	// Create a regular expression with the substring to remove and the 'g' flag for global search
...
function CN_StartSpeechRecognition() {
	...
	CN_SPEECHREC.onresult = (event) => {
		...
		if(final_transcript.trim() == "m42" || final_transcript.trim() == "m 42 copy" || final_transcript.trim() == "yem 42" || final_transcript.trim() == "yum 42") {
			CN_SayOutLoud("Hello Sir");
			go = true;
			return;
		}else if(!final_transcript.includes("m42") && !final_transcript.includes("hey m42") && go == false){
			console.log("No copy detected, ignoring");
			return
		}

		final_transcript = final_transcript.replace('hey ','');
		final_transcript = final_transcript.replace('hi ','');
		final_transcript = final_transcript.replace('hello ','');

		final_transcript = removeSubstringFromString(final_transcript, "m42");


		go = false

		if(final_transcript.trim() == "m42" || final_transcript.trim() == "" || final_transcript.trim() == ","){
			return
		}


		// final_transcript = removeSubstringFromString(final_transcript, "hey copy");
		// final_transcript = removeSubstringFromString(final_transcript, "copy");
	...
	jQuery("body").append(
		"<div style='position: fixed; top: 8px; right: 16px; display: inline-block; " +
			...
			"<div style='padding: 4px 40px; border-bottom: 1px solid grey;'>" +
				"<a href='https://www.linkedin.com/in/ajun-ravi-520760166/' " +
					"style='display: inline-block; font-size: 16px; line-height: 80%; padding: 8px 0; color: #8293a1;' " +
					"target=_blank title='Ajun's LinkedIn'>Ajun Ravi<br />" +
				"</a>" +
			"</div>" +
...
function CN_OnSettingsIconClick() {
	...
	var desc = "<div style='text-align: left; margin: 8px;'></div>";
	...
	// Add donations frame
	var donations = "<br/><h2></h2><p style='font-size: 15px; margin-top: 15px;'>";
	...
	jQuery("body").append("<div style='background: rgba(0,0,0,0.8); position: absolute; overflow-y: auto; top: 0; right: 0; left: 0; bottom: 0; z-index: 999999; padding: 20px; color: white; font-size: 13px;' id='TTGPTSettingsArea'>" +
		"<div style='width: 600px; margin-left: auto; margin-right: auto; overflow-y: auto;'><h1>⚙️ Ajun's Voice-42 ChatGPT settings</h1>"+desc+rows+donations+"</div></div>");
